...
export function ResultsPanel({ filteredMeals, restaurantById, restaurantsCount, totalEligibleMeals, selectedMealKey, onSelectMeal, onResetFilters, sortKey, sortDirection, onSort, loading, error }: { filteredMeals: Meal[]; restaurantById: Map<string, Restaurant>; restaurantsCount: number; totalEligibleMeals: number; selectedMealKey: string | null; onSelectMeal: (meal: Meal) => void; onResetFilters: () => void; sortKey: SortKey; sortDirection: SortDirection; onSort: (key: SortKey) => void; loading: boolean; error: string | null }) {
  ...
  return <div className="results-panel"><div className="column-head"><span>Meal</span><SortHeader activeKey={sortKey} direction={sortDirection} label="Cal" sortKey="calories" onSort={onSort} /><SortHeader activeKey={sortKey} direction={sortDirection} label="Protein" sortKey="protein_g" onSort={onSort} /><SortHeader activeKey={sortKey} direction={sortDirection} label="P%" sortKey="protein_pct" onSort={onSort} /><SortHeader activeKey={sortKey} direction={sortDirection} label="Carbs" sortKey="carbs_g" onSort={onSort} /><SortHeader activeKey={sortKey} direction={sortDirection} label="Fat" sortKey="fat_g" onSort={onSort} /></div>
    <div className="result-list" ref={listRef}>{filteredMeals.length === 0
      ? <div className="empty"><p>{loading ? 'Loading meals…' : error ?? 'No meals match these filters.'}</p>{!loading && !error && <button className="reset-filters" type="button" onClick={onResetFilters}>Reset filters</button>}</div>
      : <FixedSizeList height={listHeight} itemCount={filteredMeals.length} itemKey={(index) => `${filteredMeals[index].restaurant_id}-${filteredMeals[index].id}`} itemSize={ROW_HEIGHT} overscanCount={8} width="100%">
        {({ index, style }: { index: number; style: CSSProperties }) => { const meal = filteredMeals[index]; return <MealRow meal={meal} restaurant={restaurantById.get(meal.restaurant_id)} selected={selectedMealKey === `${meal.restaurant_id}-${meal.id}`} style={style} onSelect={onSelectMeal} /> }}
      </FixedSizeList>}</div>
...
function MealRow({ meal, restaurant, selected, style, onSelect }: { meal: Meal; restaurant?: Restaurant; selected: boolean; style: CSSProperties; onSelect: (meal: Meal) => void }) {
  ...
  return <div aria-label={`${meal.name}, ${restaurantName}, ${formatNumber(meal.calories, false, 0)} calories, ${formatNumber(meal.protein_g, false, 1)} grams protein${meal.is_estimate ? ' estimated' : ''} — focus on map`} className={`meal-row ${selected ? 'selected' : ''}`} role="button" style={style} tabIndex={0} onClick={() => onSelect(meal)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect(meal) } }}>
